...
function controllo(lista, test, somma){
    return lista.filter(elemento => test(elemento)).reduce(somma, 0)
}
...
function lista(frasi, analisi){
    return frasi.map(frase => analisi(frase))
}
...
function numbers(numeri, condizione){
    return numeri.map((x) => (condizione(x) ? x*2 : x ))
}
...
function esercizio(oggetti, callback, categoria){
    const result = {}
...
// secondo metodo
function esercizio (oogetti, callback, categoria){
...
function esercizio(listaGenerica, listaFunzioni){
    // return listaGenerica.filter(x=> listaFunzioni.every(y => y(x)))
    let result = []
    listaGenerica.forEach(x => {listaFunzioni.forEach(y => y(x)) 
        result.push(x)
    })
    return result
}
